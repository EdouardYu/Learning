...
module.exports = (app) => {
  app.get("/api/pokemons", auth, (req, res) => {
    ...
    if (req.query.name) {
      ...
      Pokemon.find({ name: { $regex: ".*" + name + ".*", $options: "i" } })
        .sort({ name: "asc" })
        .limit(limit)
        .then((pokemons) => {
          ...
          Pokemon.countDocuments({
            ...
            .then((count) => {
              message = `Une liste de ${pluralize(
                count,
                "pokémon",
                "pokémons"
              )} correspond au terme de la recherche '${name}'.`;
              return res.json(success(message, pokemons));
            })
            .catch(() => {
              message =
                "Erreur 500 : Aucune liste de pokémons n'a pu être récupérée. Réessayez plus tard.";
