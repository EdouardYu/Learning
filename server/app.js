...
//Ou
app
  .use(favicon(__dirname + "/pokeball.ico"))
  .use(bodyParser.json())
  .use(cors());

app.get("/", (req, res) => res.send("Pokédex API")); // coeur de express : '/' est la route par défaut vers l'application
...
app.use(({ res }) => {
  const message =
    "Erreur 404 : Impossible de trouver la ressource demandée ! Vous pouvez essayer une autre URL";
  res.status(404).json({ message });
});

app.listen(port, () =>
  console.log(`L'application Node est démarrée sur : http://localhost:${port}`)
